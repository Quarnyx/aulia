<form id="tambah-barang" enctype="multipart/form-data">
    ...
    $sql = "SELECT * FROM stok_masuk WHERE id_masuk = '$_POST[id]'";
    ...
    <input type="hidden" name="id" value="<?= $rowa['id_masuk'] ?>">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <option value="<?= $row['id_barang'] ?>" data-hargabeli="<?= $row['harga_beli'] ?>"
                                <?= $row['id_barang'] == $rowa['id_barang'] ? 'selected' : '' ?>>
                                <?= $row['nama_barang'] ?>
                            </option>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <textarea class="form-control" name="keterangan" id="keterangan"
                    placeholder="Keterangan"><?= $rowa['keterangan'] ?></textarea>
            </div>
        </div>
    </div>
...
<script>
    ...
    $("#tambah-barang").submit(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "pages/barang-masuk/proses-barang-masuk.php?aksi=update-barang-masuk",
            data: formData,
            contentType: false,
            processData: false,
